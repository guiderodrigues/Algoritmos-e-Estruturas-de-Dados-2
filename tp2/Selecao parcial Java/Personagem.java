...
public class Personagem {
  ...
  public static void partialSelectionSort(Personagem[] personagens, int k) {
    for (int i = 0; i < k; i++) {
        int menor = i;
        for (int j = i + 1; j < personagens.length; j++) {
            if (personagens[j] != null && personagens[menor] != null) {
                int dateComparison = personagens[menor].dateOfBirth.compareTo(personagens[j].dateOfBirth);
                if (dateComparison > 0 || (dateComparison == 0 && personagens[menor].name.compareTo(personagens[j].name) > 0)) {
                    menor = j;
                }
            }
        }
  ...
  public static void main(String[] args) {
    ...
    partialSelectionSort(Personagens,10);
    for (int i = 0; i < 10; i++) {
