...
void ler(Personagem *personagens[], const char *arquivo, const char *id) {
...
void mostrar(Personagem *personagens) {
   ...
   sprintf(output, "[%s ## %s ## {%s} ## %s ## %s ## %s ## %s ## %s ## %s ## %s ## %s ## %s ## %s ## %s ## %s ## %s ## %s]\n",
        ...
        personagens->species, personagens->patronus, personagens->hogwartsStaff ? "true" : "false", personagens->hogwartsStudent ? "true" : "false", personagens->actorName, personagens->alive ? "true" : "false",
        personagens->dateOfBirth, personagens->yearOfBirth, personagens->eyeColour, personagens->gender,
        personagens->hairColour, personagens->wizard ? "true" : "false");
...
int comp = 0;
int trocas = 0;
void recursiveSelectionSort(Personagem personagens[], int quant, int index)
{
    if (index >= quant - 1)
    {
        comp++;
    ...
    int minIndex = index;
    for (int i = index + 1; i < quant; i++)
    {
        comp++;
        if (strcmp(personagens[i].name, personagens[minIndex].name) < 0)
        {
            comp++;
            minIndex = i;
            trocas++;
    ...
    if (minIndex != index)
    {
        swap(&personagens[index], &personagens[minIndex]);
    }

    recursiveSelectionSort(personagens, quant, index + 1);
}
...
int main() {
  ...
  while(strcmp(id, "FIM") != 0) {
    printf("teste main");
    ler(personagens, "/tmp/characters.csv", id);
    scanf("%s", id);
  }
  recursiveSelectionSort(personagens, n, 0);
  for(int i = 0; i < n; i++) {
    if(personagens[i] != NULL) {
      mostrar(personagens[i]);
    }
