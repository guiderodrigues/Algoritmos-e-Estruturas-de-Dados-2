...
Úlitma atualização: 30/04/2024
Objetivo: Organizar os personagens de Harry Potter em ordem alfabetica por cor de cabelo.
*/
...
public static void heapsort(Personagem[] personagens, int i, int n){
  int largest = i;
  int esq = 2*i + 1;
  int dir = 2*i + 2;
  comp++;
  if(esq<n && (personagens[largest].hairColour.compareTo(personagens[esq].hairColour) < 0 ||
   (personagens[largest].hairColour.compareTo(personagens[esq].hairColour) == 0 &&
   personagens[largest].name.compareTo(personagens[esq].name) < 0))){
      largest = esq;
  }
  comp++;
  if(dir<n && (personagens[largest].hairColour.compareTo(personagens[dir].hairColour) < 0 ||
   (personagens[largest].hairColour.compareTo(personagens[dir].hairColour) == 0 && 
   personagens[largest].name.compareTo(personagens[dir].name) < 0))){
      largest = dir;
  }
  comp++;
  if(largest != i){
      Personagem aux = personagens[i];
      personagens[i] = personagens[largest];
      personagens[largest] = aux;

      trocas+=3;

      heapsort(personagens, largest, n);
  }
}
  ...
  for(int i = quant-1;i>0;i--){
      ...
      Personagens[i] = tmp;
      trocas+=3;
      heapsort(Personagens, 0, i);
  }
  long endTime = System.nanoTime();
        long duration = (endTime - startTime);
        try{
            RandomAccessFile raf = new RandomAccessFile("matrícula_heapsort.txt", "rw");
            raf.writeBytes("806574" + "\t" + comp + "\t" + trocas + "\t" + duration);
            raf.close();
        }catch (IOException e){
