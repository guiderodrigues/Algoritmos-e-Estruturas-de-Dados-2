...
public class Personagem {
  ...
  public static void countingsort(Personagem[] personagens, int quant) {
    int maiorAltura = getMaior(personagens, quant);

    int[] count = new int[maiorAltura + 1];
    Personagem[] ordenado = new Personagem[quant];
    ...
    for (int i = quant - 1; i >= 0; i--) {
      ordenado[count[personagens[i].getAno()] - 1] = personagens[i];
      count[personagens[i].getAno()]--;
    }

    for (int i = 0; i < quant - 1; i++) {
      for (int j = i + 1; j < quant; j++) {
        if (ordenado[i].getAno() == ordenado[j].getAno()) {
          if (ordenado[i].getNome().compareTo(ordenado[j].getNome()) > 0) {
            Personagem temp = ordenado[i];
  ...
  public static int getMaior(Personagem[] personagens, int quant) {
    int maiorAltura = personagens[0].getAno();
    for (int i = 1; i < quant; i++) {
      if (personagens[i].getAno() > maiorAltura) {
        maiorAltura = personagens[i].getAno();
      }
    }
    return maiorAltura;
  }
  ...
  public static void main(String[] args) {
    ...
    countingsort(Personagens, quant);
    for (int i = 0; i < quant; i++) {
